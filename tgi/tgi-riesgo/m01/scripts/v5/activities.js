...
function Activities() {
    ...
    var dragtotal = 0;
    var timers = [];
    this.flagMillonario = false;
    this.indexMillonario = 1;
    ...
    this.TryAgain = function() {
        ...
        $(".x-answer").removeClass("qselected notcorrect");
        main.stopMedia();
    ...
    this.droppedFeed = function(event, ui) {
        ...
        if ($source.data('option') == $target.data('option')) {
            ...
            console.log("#feedback_0" + suma103);
            $("#x-feedback_panel").html($("#feedback_0" + suma103).html());
            $("#x-feedback_panel").fadeIn();
    ...
    this.millonarioEntry = function(e) {
        e.preventDefault();
        var count_question = $(".n0").length;

        if(!_self.flagMillonario) {

            $('.n0' + _self.indexMillonario).find(".x-on-millonario-middle").css("visibility", "hidden");
            $('.n0' + _self.indexMillonario).find(".x-on-millonario-friend").css("visibility", "hidden");
            _self.flagMillonario = true;

            if($(this).hasClass("q-correct")) {
                $(this).addClass('ansRight');
                main.audioPlay('#correct_sound');
            }
            else {
            ...
            var tid = setTimeout(function () {
                if((_self.indexMillonario + 1) <= count_question) {
                    _self.indexMillonario += 1;
                    $('.namba').removeClass('disFlex');
                    $('.n0' + _self.indexMillonario).addClass('disFlex');
                    _self.flagMillonario = false;
                }
                else {
                    $('.x-feedback_good').addClass('disFlex');
                }
            }, 3000);
    ...
    this.millonarioMiddle = function(e) {
        e.preventDefault();
        var incorrect_option = new Array($('.n0' + _self.indexMillonario).find(".q-option").length - 1);
        var index = 0;
        $('.n0' + _self.indexMillonario).find(".q-option").each(function(i,e){
            if(!$(e).hasClass("q-correct")) {
                incorrect_option[index] = i;
                index += 1;
            }
        });

        incorrect_option.sort(() => Math.random() - 0.5);
        $('.n0' + _self.indexMillonario).find(".q-option").eq(incorrect_option[0]).css("visibility", "hidden");
        $('.n0' + _self.indexMillonario).find(".q-option").eq(incorrect_option[1]).css("visibility", "hidden");
        $(this).css("visibility", "hidden");
        e.stopImmediatePropagation();

    }

    this.millonarioFriend = function(e) {

        e.preventDefault();
        var count_question = $(".n0").length;

        $('.n0' + _self.indexMillonario).find(".q-correct").addClass("ansFriend");
        _self.flagMillonario = true;

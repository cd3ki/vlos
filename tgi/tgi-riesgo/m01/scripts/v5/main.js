...
$(document).on('click', '.x-on_top_close_only1', main.closeOnTop_only1);
...
$(document).on('click', '.x-on-millonario', activities.millonarioEntry);
$(document).on('click', '.x-on-millonario-middle', activities.millonarioMiddle);
$(document).on('click', '.x-on-millonario-friend', activities.millonarioFriend);
...
this.closeOnTop_only1 = function() {
    $('.x-on_top_content').fadeOut(function () {
        $('.x-on_top_content').remove();
        main.stopMedia();
        main.audioPlay("#closepopup_sound");
...
this.buildMenu = function(){
    var itemSec = 1;
    var menuStr = "<button class='dk-menu-dialog-close closePopUp'><i class='fa fa-times fa-2x'></i></button>";
    for (var i=0; i < jsonCourse.modules.length; i++) {
    ...
    for (var i=0; i < jsonCourse.modules.length; i++) {
        for (var j = 0; j<jsonCourse.modules[i].submodules.length; j++) {
            if(actualView == jsonCourse.modules[i].submodules[j].code){
                ...
                return "<b>"+jsonCourse.modules[i].submodules[j].title+"</b>";
            }
        }
    }
